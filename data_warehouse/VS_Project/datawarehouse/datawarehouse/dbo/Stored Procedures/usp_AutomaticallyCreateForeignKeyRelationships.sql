...
exec util.usp_DropAllForeignKeyConstraints


exec [util].[usp_AutomaticallyCreateForeignKeyRelationships_All]


exec util.usp_AutomaticallyCreateForeignKeyRelationships @verbose = 1, @ExecSql = 1


exec util.usp_AutomaticallyCreateForeignKeyRelationships @verbose = 1, @ExecSql = 0


exec util.usp_AutomaticallyCreateForeignKeyRelationships @verbose = 0, @ExecSql = 1


exec util.usp_AutomaticallyCreateForeignKeyRelationships @verbose = 0, @ExecSql = 0
...
select
	...
	,case
		when t.TABLE_NAME like 'fact%' OR t.TABLE_NAME like 'matrix%' then cast(1 as bit)
		else cast(0 as bit)
	end as IsFact
	,case
		when c.COLUMN_NAME like '%!_%' escape '!' then left(c.COLUMN_NAME, CHARINDEX('_', c.COLUMN_NAME) -1)
		else c.COLUMN_NAME
	end as DimKeyName
into #cols
from
...
where
	...
	and (
		...
		or (t.TABLE_NAME like 'fact%' OR t.TABLE_NAME like 'matrix%')
	)
